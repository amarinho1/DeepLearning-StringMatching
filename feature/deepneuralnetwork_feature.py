...
def deep_neural_net_gru(train_data_1, train_data_2, train_data_3, train_labels, test_data_1, test_data_2, test_data_3, test_labels, max_len,
    ...
    right_branch.add(Dropout(0.01))
     # definition for feature branch of the network
    feature_branch = Sequential(name="f_branch")
    feature_branch.add( Masking( mask_value=0 , input_shape=(max_len,), name="fm_branch") )
    feature_branch.add(Dropout(0.01))
    print(feature_branch.summary())
    ...
    else: final_model.add(Merge([con_layer, mul_layer, dif_layer, feature_branch], mode='concat',name="merge_threeconcat")) 
    ...
    print('Fitting...')
    final_model.fit([train_data_1, train_data_2, train_data_3], train_labels, verbose = 0, validation_data=([test_data_1, test_data_2,test_data_3], test_labels), 
		    callbacks=[checkpointer, early_stop],nb_epoch=20)
    start_time = time.time()
    aux = final_model.predict_classes([test_data_1, test_data_2, test_data_3], verbose = 0).ravel()
...
def evaluate_deep_neural_net(dataset='dataset-string-similarity.txt', method='gru', training_instances=-1,
                             bidirectional=True, hiddenunits=60, selfattention=True , maxpooling=True , 
                        alignment = True , shortcut=True , multiplerlu=True , onlyconcat=False):
    max_seq_len = 40
    num_true = 0.0
    num_false = 0.0
    ...
    with open(dataset) as csvfile:
        ...
        for row in reader:
            ...
            row['s3'] = bytearray(unicodedata.normalize('NFKD', (u'|' + row['s3'] + u'|')), encoding='utf-8')
    ...
    for i, example in enumerate(XB1):
        for t, char in enumerate(example):
            if t < max_seq_len:
                ...
                break
    XB1 = aux1
    aux1 = np.memmap("/home/amarinho/data-amarinho/temporary-file-dnn-3-" + method, mode="w+", shape=(len(XC1), max_seq_len),
                     dtype=np.bool)
    for i, example in enumerate(XC1):
        for t, char in enumerate(example):
            if t < max_seq_len:
                aux1[i, t] = 1
            else:
                break
    XC1 = aux1
    aux1 = np.memmap("/home/amarinho/data-amarinho/temporary-file-dnn-4-" + method, mode="w+", shape=(len(XA2), max_seq_len, len(chars)),
                     dtype=np.bool)
    for i, example in enumerate(XA2):
        for t, char in enumerate(example):
            if t < max_seq_len:
                ...
                break
    XA2 = aux1
    aux1 = np.memmap("/home/amarinho/data-amarinho/temporary-file-dnn-5-" + method, mode="w+", shape=(len(XB2), max_seq_len, len(chars)),
                     dtype=np.bool)
    for i, example in enumerate(XB2):
        for t, char in enumerate(example):
            if t < max_seq_len:
                ...
                break
    XB2 = aux1
    aux1 = np.memmap("/home/amarinho/data-amarinho/temporary-file-dnn-6-" + method, mode="w+", shape=(len(XC2),max_seq_len),
                     dtype=np.bool)
    for i, example in enumerate(XC2):
        for t, char in enumerate(example):
            if t < max_seq_len:
                aux1[i, t] = 1
            else:
    ...
    os.remove("/home/amarinho/data-amarinho/temporary-file-dnn-5-" + method)
    os.remove("/home/amarinho/data-amarinho/temporary-file-dnn-6-" + method)
    sys.stdout.flush()
